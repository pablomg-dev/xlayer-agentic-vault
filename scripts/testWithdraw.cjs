...
const WITHDRAW_AMOUNT = BigInt("1000000000000000"); // 0.001 OKB
...
async function main() {
  ...
  const walletBalanceBefore = await publicClient.getBalance({ address: walletAddress });
  console.log("Wallet balance BEFORE:", walletBalanceBefore.toString(), "wei");

  console.log("\nWithdrawing", WITHDRAW_AMOUNT.toString(), "wei...");
  ...
  console.log("Withdraw confirmed! Block:", receipt.blockNumber.toString());
  ...
  const walletBalanceAfter = await publicClient.getBalance({ address: walletAddress });
  console.log("Wallet balance AFTER:", walletBalanceAfter.toString(), "wei");
}
